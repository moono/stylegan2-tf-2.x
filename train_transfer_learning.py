...
from tf_utils import check_tf_version, allow_memory_growth
from load_models import load_generator, load_discriminator
from dataset_cartoon_set import get_cartoonset_dataset
...
def initiate_models(g_params, d_params, ckpt_dir, use_custom_cuda):
    discriminator = load_discriminator(d_params, ckpt_dir=ckpt_dir, custom_cuda=use_custom_cuda)
    generator = load_generator(g_params=g_params, is_g_clone=False, ckpt_dir=ckpt_dir, custom_cuda=use_custom_cuda)
    g_clone = load_generator(g_params=g_params, is_g_clone=True, ckpt_dir=ckpt_dir, custom_cuda=use_custom_cuda)
...
class Trainer(object):
    def __init__(self, t_params, name):
        ...
        self.official_weights_dir = t_params['official_weights_dir']
...
def main():
    ...
    parser.add_argument('--official_weights_dir', default='./official-converted/cuda', type=str)
    parser.add_argument('--images_dir', default='/home/mookyung/Downloads/cartoonset10k', type=str)
    parser.add_argument('--train_res', default=1024, type=int)
    parser.add_argument('--shuffle_buffer_size', default=100, type=int)
    parser.add_argument('--batch_size_per_replica', default=1, type=int)
    ...
    # prepare dataset
    dataset = get_cartoonset_dataset(args['images_dir'], args['train_res'],
                                     batch_size=global_batch_size, epochs=None, buffer_size=args['shuffle_buffer_size'])
    ...
    with strategy.scope():
        ...
        training_parameters = {
            ...
            'official_weights_dir': args['official_weights_dir'],
            ...
            'n_total_image': 312000,    # 25000000,
            'n_samples': 3,
            'train_res': args['train_res'],
        }

        trainer = Trainer(training_parameters, name=f'cartoonset10k-{args["train_res"]}x{args["train_res"]}')
